...
def main():
	...
	# create a dict of keywords from the HTML, with words as keys and values as number of occurrences in the HTML
	...
	#Make a new dict for the keywords, but set up their values as a distribution to later randomly pick from
	...
	#narrow down keywords by selecting keywords at random, giving weight to ones with high occurrence.
	for i in range(0,3):
	...
	# Narrow down the personality descriptions by chosing randomly
	random_descriptions = []
	...
	closeness = {}
	# Run API calls to assess relatedness of selected keywords to personality words
